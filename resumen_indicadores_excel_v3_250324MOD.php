...
//require_once 'bd.php';  
function conectarBD(){
  $cnn = pg_connect("host=localhost port=5432 dbname=sigresmtn250324 user=postgres password=12345") or die("No se pudo conectar con el servidor");
  return $cnn;
}
...
$objPHPExcel->getProperties()->setCreator("Frank Alvarez")
            ->setLastModifiedBy("Rubén Freites")
...
//$fe_ini = "2022-08-26 00:00:00";
//$fe_fin = "2023-01-26 00:00:00";
...
//***********************************************************************************/
 ...
 /*$sql = "SELECT 
  ...
  c001t_requerimiento.co_requerimiento as num_req 
...
FROM (
 SELECT
    ...
    c001t_requerimiento.tx_obs_requerimiento AS obs,
    c001t_requerimiento.co_requerimiento AS num_req,
    ROW_NUMBER() OVER (PARTITION BY c001t_requerimiento.co_requerimiento ORDER BY c001t_requerimiento.co_requerimiento) AS rn
  ...
  FROM
    ...
    public.i002t_gerencia,
    public.i001t_unidad_negocio,
    public.i020t_proceso_servicio

WHERE
  i010t_analista.co_analista = c001t_requerimiento.co_analista AND
  i006t_subproceso.co_proceso = i020t_proceso_servicio.co_proceso AND
  i020t_proceso_servicio.co_servicio = i008t_actividad.co_servicio AND
  i007t_servicio.co_servicio = i008t_actividad.co_servicio AND
  c001t_requerimiento.co_subactividad = i009t_subactividad.co_subactividad AND
  ...
  i002t_gerencia.co_gerencia = i003t_proyecto.co_gerencia AND
  i001t_unidad_negocio.co_unidad_negocio = i002t_gerencia.co_unidad_negocio AND
  c001t_requerimiento.fe_fin_requerimiento >= '$fe_ini' AND
  c001t_requerimiento.fe_fin_requerimiento <= '$fe_fin'
...
while($row = pg_fetch_array($result))
		{
			...
			//->setCellValue('G'.$fila, pg_fetch_arrayservicio)				
			->setCellValue('G'.$fila, $servicio)				
			->setCellValue('H'.$fila, $actividad)
...
$objDrawing->setPath('imagenes/titulo_mtn.png');
...
header('Content-Disposition: attachment;filename="Reporte de Gestión.xls"');
