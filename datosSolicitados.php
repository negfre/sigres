<?php
  ...
  switch ($opcion2) {
    ...
    break;     
    case 'resumenIndicadores2': 
     cargar_resumen_indicadores2(); 
    break;       
    case 'resumenMatriz': 
  ...
  function actualizarFechaCierre($nuevaFechaCierre) {
    // Conexión a la base de datos
    $con = conectar_db();
  
    // Preparar la consulta SQL
    $sql = "UPDATE i027_t_fecha_cierre SET fe_cierre = '$nuevaFechaCierre' WHERE co_fe_cierre = 1"; // Suponiendo que el ID de la fecha de cierre es 1
  ...
  function cargar_gerencia(){
    ...
    $sql = "SELECT co_gerencia, co_unidad_negocio, tx_nombre_gerencia FROM i002t_gerencia WHERE co_unidad_negocio = ".$id_un." AND in_activo='1' ORDER BY tx_nombre_gerencia";
    //echo $sql;
    $rs = consultas($sql);
    while($obj = pg_fetch_object($rs)){
      $arr[] = $obj;
  ...
  function cargar_proyecto($fecha){
    ...
    $sql = "SELECT 
        ...
        (i003t_proyecto.tx_nombre_proyecto || '  (' || i002t_gerencia.tx_nombre_gerencia || ' - ' || i001t_unidad_negocio.tx_nombre_unidad_negocio || ')') AS tx_nombre
      FROM 
    ...
    if ($_SESSION["apoyo"] == '1')
     $sql = "SELECT co_servicio, tx_nombre_servicio FROM i007t_servicio WHERE in_activo='1' ORDER BY tx_nombre_servicio";
      ...
      $sql = "select co_servicio, tx_nombre_servicio from i007t_servicio where co_servicio IN (
              ...
              where  i020t_proceso_servicio.in_activo='1' and i020t_proceso_servicio.co_proceso = (SELECT
              ...
              WHERE i006t_subproceso.co_subproceso=".$_SESSION["subproceso"].")) and i007t_servicio.in_activo='1' ORDER BY i007t_servicio.tx_nombre_servicio";
    //echo "sql:".$sql;
    $rs = consultas($sql);
    while($obj = pg_fetch_object($rs)){
      $arr[] = $obj;
  ...
  function cargar_medida(){
    ...
    $sql = "SELECT co_unidad_medida, tx_nombre_medida from i014t_unidad_medida
            ORDER BY i014t_unidad_medida.tx_nombre_medida";
    //echo $sql;
    $rs = consultas($sql);
    while($obj = pg_fetch_object($rs)){
      $arr[] = $obj;
  ...
  function cargar_lista_usuarios(){
   ...
   function cargar_fecha_cierre(){
    $sql = "SELECT 
        (i027_t_fecha_cierre.fe_cierre || ' (' || i001t_unidad_negocio.tx_nombre_unidad_negocio  || ')') as tx_nombre, 
        i002t_gerencia.co_gerencia
      FROM 
        public.i002t_gerencia, 
        public.i001t_unidad_negocio
  ...
  function cargar_resumen_indicadores2(){
  $annio = $_GET['anio'];  
    $sql = "SELECT 
       i007t_servicio.tx_nombre_servicio as indicador, 
       COUNT(c001t_requerimiento.co_requerimiento) AS servicios,
       to_char((sum(nu_tiempo_efectivo)/60), '99999.99') as hh  
    FROM 
      public.c001t_requerimiento, 
      public.i003t_proyecto, 
    ...
    WHERE ". 
        $annio." = extract(year from c001t_requerimiento.fe_fin_requerimiento)::int AND
      c001t_requerimiento.fe_fin_requerimiento >= '2017-04-01 00:00:00' AND
